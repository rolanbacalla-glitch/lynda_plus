import { E as Deferred, F as getModularInstance, O as FirebaseError, T as Component, c as _getProvider, f as _registerComponent, g as getApp, u as _isFirebaseServerApp, w as Logger, x as registerVersion } from "./index.esm-CgfkFqYz.js";
//#region node_modules/@firebase/ai/dist/esm/index.esm.js
...
* @public
*/
var AIError = class AIError extends FirebaseError {
	/**
	...
	*/
	constructor(code, message, customErrorData) {
		...
		const fullMessage = `${service}: ${message} (${`${service}/${code}`})`;
...
var VertexAIBackend = class extends Backend {
	...
	*/
	constructor(location = DEFAULT_LOCATION) {
		...
		if (!location) this.location = DEFAULT_LOCATION;
...
function encodeInstanceIdentifier(backend) {
	if (backend instanceof GoogleAIBackend) return `${AI_TYPE}/googleai`;
	else if (backend instanceof VertexAIBackend) return `${AI_TYPE}/vertexai/${backend.location}`;
	else throw new AIError(AIErrorCode.ERROR, `Invalid backend: ${JSON.stringify(backend.backendType)}`);
}
/**
...
function decodeInstanceIdentifier(instanceIdentifier) {
	...
	if (identifierParts[0] !== AI_TYPE) throw new AIError(AIErrorCode.ERROR, `Invalid instance identifier, unknown prefix '${identifierParts[0]}'`);
	switch (identifierParts[1]) {
		case "vertexai":
			const location = identifierParts[2];
			if (!location) throw new AIError(AIErrorCode.ERROR, `Invalid instance identifier, unknown location '${instanceIdentifier}'`);
			return new VertexAIBackend(location);
		case "googleai": return new GoogleAIBackend();
		default: throw new AIError(AIErrorCode.ERROR, `Invalid instance identifier string: '${instanceIdentifier}'`);
	}
}
/**
* @license
...
* possible.
*/
var ChromeAdapterImpl = class ChromeAdapterImpl {
	constructor(languageModelProvider, mode, onDeviceParams) {
		...
		if (onDeviceParams) {
			...
			if (!this.onDeviceParams.createOptions) this.onDeviceParams.createOptions = { expectedInputs: defaultExpectedInputs };
			else if (!this.onDeviceParams.createOptions.expectedInputs) this.onDeviceParams.createOptions.expectedInputs = defaultExpectedInputs;
		}
	}
	/**
	...
	*/
	async isAvailable(request) {
		...
		if (this.mode === InferenceMode.ONLY_ON_DEVICE) {
			if (availability === Availability.UNAVAILABLE) throw new AIError(AIErrorCode.API_NOT_ENABLED, "Local LanguageModel API not available in this environment.");
	...
	*/
	static isOnDeviceRequest(request) {
		...
		for (const content of request.contents) {
			...
			for (const part of content.parts) if (part.inlineData && ChromeAdapterImpl.SUPPORTED_MIME_TYPES.indexOf(part.inlineData.mimeType) === -1) {
	...
	*/
	async downloadIfAvailable() {
		...
		if (availability === Availability.DOWNLOADABLE) this.download();
		return availability;
	}
	/**
	...
	*/
	static async toLanguageModelMessageContent(part) {
		if (part.text) return {
			...
			const imageBlob = await (await fetch(`data:${part.inlineData.mimeType};base64,${part.inlineData.data}`)).blob();
			return {
				type: "image",
	...
	*/
	async createSession() {
		if (!this.languageModelProvider) throw new AIError(AIErrorCode.UNSUPPORTED, "Chrome AI requested for unsupported browser version.");
		const newSession = await this.languageModelProvider.create(this.onDeviceParams.createOptions);
		if (this.oldSession) this.oldSession.destroy();
	...
	*/
	static toStreamResponse(stream) {
		...
		return { body: stream.pipeThrough(new TransformStream({ transform(chunk, controller) {
			const json = JSON.stringify({ candidates: [{ content: {
				role: "model",
				parts: [{ text: chunk }]
			} }] });
...
function chromeAdapterFactory(mode, window, params) {
	if (typeof window !== "undefined" && mode) return new ChromeAdapterImpl(window.LanguageModel, mode, params);
}
/**
* @license
* Copyright 2024 Google LLC
...
var AIService = class {
	constructor(app, backend, authProvider, appCheckProvider, chromeAdapterFactory) {
		...
		this.auth = authProvider?.getImmediate({ optional: true }) || null;
		this.appCheck = appCheck || null;
		if (backend instanceof VertexAIBackend) this.location = backend.location;
		else this.location = "";
	}
	_delete() {
...
function factory(container, { instanceIdentifier }) {
	if (!instanceIdentifier) throw new AIError(AIErrorCode.ERROR, "AIService instance identifier is undefined.");
	const backend = decodeInstanceIdentifier(instanceIdentifier);
	return new AIService(container.getProvider("app").getImmediate(), backend, container.getProvider("auth-internal"), container.getProvider("app-check-internal"), chromeAdapterFactory);
}
/**
* @license
* Copyright 2025 Google LLC
...
function initApiSettings(ai) {
	if (!ai.app?.options?.apiKey) throw new AIError(AIErrorCode.NO_API_KEY, `The "apiKey" field is empty in the local Firebase config. Firebase AI requires this field to contain a valid API key.`);
	else if (!ai.app?.options?.projectId) throw new AIError(AIErrorCode.NO_PROJECT_ID, `The "projectId" field is empty in the local Firebase config. Firebase AI requires this field to contain a valid project ID.`);
	else if (!ai.app?.options?.appId) throw new AIError(AIErrorCode.NO_APP_ID, `The "appId" field is empty in the local Firebase config. Firebase AI requires this field to contain a valid app ID.`);
	const apiSettings = {
	...
	} else if (ai.appCheck) if (ai.options?.useLimitedUseAppCheckTokens) apiSettings.getAppCheckToken = () => ai.appCheck.getLimitedUseToken();
	else apiSettings.getAppCheckToken = () => ai.appCheck.getToken();
	if (ai.auth) apiSettings.getAuthToken = () => ai.auth.getToken();
	return apiSettings;
}
/**
* @license
...
var AIModel = class AIModel {
	...
	*/
	static normalizeModelName(modelName, backendType) {
		if (backendType === BackendType.GOOGLE_AI) return AIModel.normalizeGoogleAIModelName(modelName);
		else return AIModel.normalizeVertexAIModelName(modelName);
	}
	/**
	* @internal
	*/
	...
	*/
	static normalizeVertexAIModelName(modelName) {
		let model;
		if (modelName.includes("/")) if (modelName.startsWith("models/")) model = `publishers/google/${modelName}`;
		else model = modelName;
		else model = `publishers/google/models/${modelName}`;
		return model;
	}
};
/**
...
var RequestURL = class {
	...
	get pathname() {
		if (this.params.templateId) return `${this.params.apiSettings.backend._getTemplatePath(this.params.apiSettings.project, this.params.templateId)}:${this.params.task}`;
		else return `${this.params.apiSettings.backend._getModelPath(this.params.apiSettings.project, this.params.model)}:${this.params.task}`;
	}
	get baseUrl() {
	...
	get queryParams() {
		...
		if (this.params.stream) params.set("alt", "sse");
		return params;
	}
};
var WebSocketUrl = class {
	...
	get pathname() {
		if (this.apiSettings.backend.backendType === BackendType.GOOGLE_AI) return "ws/google.firebase.vertexai.v1beta.GenerativeService/BidiGenerateContent";
		else return `ws/google.firebase.vertexai.v1beta.LlmBidiService/BidiGenerateContent/locations/${this.apiSettings.location}`;
	}
};
/**
...
function getClientHeaders(url) {
	...
	*/
	if (url.params.apiSettings.inferenceMode === InferenceMode.PREFER_ON_DEVICE || url.params.apiSettings.inferenceMode === InferenceMode.PREFER_IN_CLOUD) loggingTags.push(HYBRID_TAG);
...
async function getHeaders(url) {
	...
	if (url.params.apiSettings.automaticDataCollectionEnabled) headers.append("X-Firebase-Appid", url.params.apiSettings.appId);
	if (url.params.apiSettings.getAppCheckToken) {
		const appCheckToken = await url.params.apiSettings.getAppCheckToken();
		if (appCheckToken) {
			headers.append("X-Firebase-AppCheck", appCheckToken.token);
			if (appCheckToken.error) logger.warn(`Unable to obtain a valid App Check token: ${appCheckToken.error.message}`);
		}
	}
	if (url.params.apiSettings.getAuthToken) {
		const authToken = await url.params.apiSettings.getAuthToken();
		if (authToken) headers.append("Authorization", `Firebase ${authToken.accessToken}`);
	}
	return headers;
}
...
function hasValidCandidates(response) {
	if (response.candidates && response.candidates.length > 0) {
		if (response.candidates.length > 1) logger.warn(`This response had ${response.candidates.length} candidates. Returning text from the first candidate only. Access response.candidates directly to use the other candidates.`);
		if (hadBadFinishReason(response.candidates[0])) throw new AIError(AIErrorCode.RESPONSE_ERROR, `Response error: ${formatBlockErrorMessage(response)}. Response body stored in error.response`, { response });
		return true;
	} else return false;
}
/**
...
function createEnhancedContentResponse(response, inferenceSource = InferenceSource.IN_CLOUD) {
	...
	*/
	if (response.candidates && !response.candidates[0].hasOwnProperty("index")) response.candidates[0].index = 0;
...
function addHelpers(response) {
	response.text = () => {
		if (hasValidCandidates(response)) return getText(response, (part) => !part.thought);
		else if (response.promptFeedback) throw new AIError(AIErrorCode.RESPONSE_ERROR, `Text not available. ${formatBlockErrorMessage(response)}`, { response });
		return "";
	};
	response.thoughtSummary = () => {
		...
		} else if (response.promptFeedback) throw new AIError(AIErrorCode.RESPONSE_ERROR, `Thought summary not available. ${formatBlockErrorMessage(response)}`, { response });
	};
	response.inlineDataParts = () => {
		if (hasValidCandidates(response)) return getInlineDataParts(response);
		else if (response.promptFeedback) throw new AIError(AIErrorCode.RESPONSE_ERROR, `Data not available. ${formatBlockErrorMessage(response)}`, { response });
	};
	response.functionCalls = () => {
		if (hasValidCandidates(response)) return getFunctionCalls(response);
		else if (response.promptFeedback) throw new AIError(AIErrorCode.RESPONSE_ERROR, `Function call not available. ${formatBlockErrorMessage(response)}`, { response });
	};
	return response;
}
/**
...
function getText(response, partFilter) {
	...
	if (response.candidates?.[0].content?.parts) {
		for (const part of response.candidates?.[0].content?.parts) if (part.text && partFilter(part)) textStrings.push(part.text);
	}
...
function getFunctionCalls(response) {
	...
	const functionCalls = [];
	if (response.candidates?.[0].content?.parts) {
		for (const part of response.candidates?.[0].content?.parts) if (part.functionCall) functionCalls.push(part.functionCall);
	}
...
function getInlineDataParts(response) {
	const data = [];
	if (response.candidates?.[0].content?.parts) {
		for (const part of response.candidates?.[0].content?.parts) if (part.inlineData) data.push(part);
	}
...
function formatBlockErrorMessage(response) {
	...
	if ((!response.candidates || response.candidates.length === 0) && response.promptFeedback) {
		...
		if (response.promptFeedback?.blockReason) message += ` due to ${response.promptFeedback.blockReason}`;
		if (response.promptFeedback?.blockReasonMessage) message += `: ${response.promptFeedback.blockReasonMessage}`;
...
async function handlePredictResponse(response) {
	...
	if (!responseJson.predictions || responseJson.predictions?.length === 0) throw new AIError(AIErrorCode.RESPONSE_ERROR, "No predictions or filtered reason received from Vertex AI. Please report this issue with the full error details at https://github.com/firebase/firebase-js-sdk/issues.");
	for (const prediction of responseJson.predictions) if (prediction.raiFilteredReason) filteredReason = prediction.raiFilteredReason;
	else if (prediction.mimeType && prediction.bytesBase64Encoded) images.push({
		mimeType: prediction.mimeType,
		bytesBase64Encoded: prediction.bytesBase64Encoded
	});
	else if (prediction.mimeType && prediction.gcsUri) images.push({
		mimeType: prediction.mimeType,
		gcsURI: prediction.gcsUri
	});
	else if (prediction.safetyAttributes);
	else throw new AIError(AIErrorCode.RESPONSE_ERROR, `Unexpected element in 'predictions' array in response: '${JSON.stringify(prediction)}'`);
	return {
		images,
		filteredReason
	};
...
function mapGenerateContentRequest(generateContentRequest) {
	generateContentRequest.safetySettings?.forEach((safetySetting) => {
		if (safetySetting.method) throw new AIError(AIErrorCode.UNSUPPORTED, "SafetySetting.method is not supported in the the Gemini Developer API. Please remove this property.");
	});
...
function mapGenerateContentCandidates(candidates) {
	...
	if (mappedCandidates) candidates.forEach((candidate) => {
		let citationMetadata;
		if (candidate.citationMetadata) citationMetadata = { citations: candidate.citationMetadata.citationSources };
		if (candidate.safetyRatings) mappedSafetyRatings = candidate.safetyRatings.map((safetyRating) => {
			return {
				...safetyRating,
		...
		if (candidate.content?.parts?.some((part) => part?.videoMetadata)) throw new AIError(AIErrorCode.UNSUPPORTED, "Part.videoMetadata is not supported in the Gemini Developer API. Please remove this property.");
...
async function processStreamInternal(stream, apiSettings, inferenceSource) {
	...
	const { value } = await streamForPeek.getReader().read();
	return {
		firstValue: value,
...
async function getResponsePromise(stream, apiSettings, inferenceSource) {
	...
	while (true) {
		...
		if (done) {
			...
			if (apiSettings.backend.backendType === BackendType.GOOGLE_AI) generateContentResponse = mapGenerateContentResponse(generateContentResponse);
...
async function* generateResponseSequence(stream, apiSettings, inferenceSource) {
	...
	while (true) {
		...
		if (apiSettings.backend.backendType === BackendType.GOOGLE_AI) enhancedResponse = createEnhancedContentResponse(mapGenerateContentResponse(value), inferenceSource);
		else enhancedResponse = createEnhancedContentResponse(value, inferenceSource);
		const firstCandidate = enhancedResponse.candidates?.[0];
		if (!firstCandidate?.content?.parts && !firstCandidate?.finishReason && !firstCandidate?.citationMetadata && !firstCandidate?.urlContextMetadata) continue;
...
function aggregateResponses(responses) {
	const aggregatedResponse = { promptFeedback: responses[responses.length - 1]?.promptFeedback };
	for (const response of responses) if (response.candidates) for (const candidate of response.candidates) {
		...
		if (typeof urlContextMetadata === "object" && urlContextMetadata !== null && Object.keys(urlContextMetadata).length > 0) aggregatedResponse.candidates[i].urlContextMetadata = urlContextMetadata;
		if (candidate.content) {
			...
			for (const part of candidate.content.parts) {
				...
				if (Object.keys(newPart).length > 0) aggregatedResponse.candidates[i].content.parts.push(newPart);
			}
		}
	}
...
async function callCloudOrDevice(request, chromeAdapter, onDeviceCall, inCloudCall) {
	...
	switch (chromeAdapter.mode) {
		...
		case InferenceMode.ONLY_IN_CLOUD: return {
			response: await inCloudCall(),
			inferenceSource: InferenceSource.IN_CLOUD
		};
		case InferenceMode.PREFER_IN_CLOUD: try {
			return {
				response: await inCloudCall(),
				inferenceSource: InferenceSource.IN_CLOUD
			};
		} catch (e) {
			if (e instanceof AIError && errorsCausingFallback.includes(e.code)) return {
				response: await onDeviceCall(),
				inferenceSource: InferenceSource.ON_DEVICE
			};
			throw e;
		...
		default: throw new AIError(AIErrorCode.ERROR, `Unexpected infererence mode: ${chromeAdapter.mode}`);
	}
}
/**
* @license
...
async function generateContentStreamOnCloud(apiSettings, model, params, singleRequestOptions) {
	if (apiSettings.backend.backendType === BackendType.GOOGLE_AI) params = mapGenerateContentRequest(params);
	return makeRequest({
...
async function templateGenerateContent(apiSettings, templateId, templateParams, singleRequestOptions) {
	return { response: createEnhancedContentResponse(await processGenerateContentResponse(await makeRequest({
		task: "templateGenerateContent",
		templateId,
		apiSettings,
		stream: false,
		singleRequestOptions
	}, JSON.stringify(templateParams)), apiSettings)) };
}
async function templateGenerateContentStream(apiSettings, templateId, templateParams, singleRequestOptions) {
	return processStream(await makeRequest({
...
async function generateContent(apiSettings, model, params, chromeAdapter, singleRequestOptions) {
	...
	return { response: createEnhancedContentResponse(await processGenerateContentResponse(callResult.response, apiSettings), callResult.inferenceSource) };
}
async function processGenerateContentResponse(response, apiSettings) {
	const responseJson = await response.json();
	if (apiSettings.backend.backendType === BackendType.GOOGLE_AI) return mapGenerateContentResponse(responseJson);
...
function assignRoleToPartsAndValidateSendMessageRequest(parts) {
	...
	if (hasUserContent && hasFunctionContent) throw new AIError(AIErrorCode.INVALID_CONTENT, "Within a single message, FunctionResponse cannot be mixed with other type of Part in the request for sending chat message.");
	if (!hasUserContent && !hasFunctionContent) throw new AIError(AIErrorCode.INVALID_CONTENT, "No Content is provided for sending chat message.");
...
function validateChatHistory(history) {
	...
	for (const currContent of history) {
		...
		if (!prevContent && role !== "user") throw new AIError(AIErrorCode.INVALID_CONTENT, `First Content should be with role 'user', got ${role}`);
		if (!POSSIBLE_ROLES.includes(role)) throw new AIError(AIErrorCode.INVALID_CONTENT, `Each item should include role field. Got ${role} but valid roles are: ${JSON.stringify(POSSIBLE_ROLES)}`);
		if (!Array.isArray(parts)) throw new AIError(AIErrorCode.INVALID_CONTENT, `Content should have 'parts' property with an array of Parts`);
		if (parts.length === 0) throw new AIError(AIErrorCode.INVALID_CONTENT, `Each Content should have at least one part`);
		const countFields = {
			text: 0,
			inlineData: 0,
			functionCall: 0,
		...
		for (const key of VALID_PART_FIELDS) if (!validParts.includes(key) && countFields[key] > 0) throw new AIError(AIErrorCode.INVALID_CONTENT, `Content with role '${role}' can't contain '${key}' part`);
		if (prevContent) {
			if (!VALID_PREVIOUS_CONTENT_ROLES[role].includes(prevContent.role)) throw new AIError(AIErrorCode.INVALID_CONTENT, `Content with role '${role}' can't follow '${prevContent.role}'. Valid previous roles: ${JSON.stringify(VALID_PREVIOUS_CONTENT_ROLES)}`);
		}
...
var ChatSession = class {
	...
	async sendMessage(request, singleRequestOptions) {
		...
		*/
		...
		this._sendPromise = this._sendPromise.then(async () => {
			...
			const functionCallMaxTurns = this.requestOptions?.maxSequentalFunctionCalls ?? DEFAULT_MAX_SEQUENTIAL_FUNCTION_CALLS;
			do {
				let formattedContent;
				if (functionCalls) {
					functionCallTurnCount++;
					formattedContent = formatNewContent(await this._callFunctionsAsNeeded(functionCalls));
				} else formattedContent = formatNewContent(request);
				...
				if (result) {
					...
					if (result.response.candidates && result.response.candidates.length > 0) {
						...
						if (blockErrorMessage) logger.warn(`sendMessage() was unsuccessful. ${blockErrorMessage}. Inspect response object for details.`);
					}
				} else functionCalls = void 0;
			} while (functionCalls && functionCallTurnCount < functionCallMaxTurns);
			if (functionCalls && functionCallTurnCount >= functionCallMaxTurns) logger.warn(`Automatic function calling exceeded the limit of ${functionCallMaxTurns} function calls. Returning last model response.`);
		});
		await this._sendPromise;
		...
		*/
		...
		const callGenerateContentStream = async () => {
			...
			do {
				...
				if (functionCalls && result.firstValue && result.firstValue.candidates && result.firstValue.candidates.length > 0) {
					const responseContent = { ...result.firstValue.candidates[0].content };
		...
		this._sendPromise = this._sendPromise.then(async () => streamPromise).catch((_ignored) => {
			...
			if (response.candidates && response.candidates.length > 0) {
				...
				if (blockErrorMessage) logger.warn(`sendMessageStream() was unsuccessful. ${blockErrorMessage}. Inspect response object for details.`);
			}
		}).catch((e) => {
			if (e.message !== SILENT_ERROR && e.name !== "AbortError") logger.error(e);
		});
	...
	*/
	_getCallableFunctionCalls(response) {
		...
		if (!functionDeclarationsTool?.functionDeclarations) return;
		const functionCalls = getFunctionCalls(response);
		if (!functionCalls) return;
		for (const functionCall of functionCalls) if (!functionDeclarationsTool.functionDeclarations?.some((declaration) => declaration.name === functionCall.name && typeof declaration.functionReference === "function")) return;
	...
	*/
	async _callFunctionsAsNeeded(functionCalls) {
		const activeCallList = /* @__PURE__ */ new Map();
		const promiseList = [];
		const functionDeclarationsTool = this.params?.tools?.find((tool) => tool.functionDeclarations);
		if (functionDeclarationsTool && functionDeclarationsTool.functionDeclarations) {
			...
			for (const [name, callData] of activeCallList) functionResponseParts.push({ functionResponse: {
				name,
				response: await callData.results
			} });
			return functionResponseParts;
		} else throw new AIError(AIErrorCode.REQUEST_ERROR, `No function declarations were provided in "tools".`);
	}
};
/**
* @license
...
async function countTokens(apiSettings, model, params, chromeAdapter, requestOptions) {
	if (chromeAdapter?.mode === InferenceMode.ONLY_ON_DEVICE) throw new AIError(AIErrorCode.UNSUPPORTED, "countTokens() is not supported for on-device models.");
...
function validateGenerationConfig(generationConfig) {
	if (generationConfig.thinkingConfig?.thinkingBudget != null && generationConfig.thinkingConfig?.thinkingLevel) throw new AIError(AIErrorCode.UNSUPPORTED, `Cannot set both thinkingBudget and thinkingLevel in a config.`);
	if (generationConfig.responseSchema != null && generationConfig.responseJsonSchema != null) throw new AIError(AIErrorCode.UNSUPPORTED, `Cannot set both responseSchema and responseJsonSchema in a config.`);
	if ((generationConfig.responseSchema != null || generationConfig.responseJsonSchema != null) && generationConfig.responseMimeType) throw new AIError(AIErrorCode.UNSUPPORTED, `responseMimeType must be set if responseSchema or responseJsonSchema are set.`);
}
/**
* @license
* Copyright 2025 Google LLC
...
var LiveSession = class {
	...
	*/
	async send(request, turnComplete = true) {
		if (this.isClosed) throw new AIError(AIErrorCode.REQUEST_ERROR, "This LiveSession has been closed and cannot be used.");
		const message = { clientContent: {
			turns: [formatNewContent(request)],
			turnComplete
		} };
	...
	*/
	async sendAudioRealtime(blob) {
		if (this.isClosed) throw new AIError(AIErrorCode.REQUEST_ERROR, "This LiveSession has been closed and cannot be used.");
		const message = { realtimeInput: { audio: blob } };
		this.webSocketHandler.send(JSON.stringify(message));
	}
	/**
	...
	*/
	async sendVideoRealtime(blob) {
		if (this.isClosed) throw new AIError(AIErrorCode.REQUEST_ERROR, "This LiveSession has been closed and cannot be used.");
		const message = { realtimeInput: { video: blob } };
		this.webSocketHandler.send(JSON.stringify(message));
	}
	/**
	...
	*/
	async *receive() {
		if (this.isClosed) throw new AIError(AIErrorCode.SESSION_CLOSED, "Cannot read from a Live session that is closed. Try starting a new Live session.");
		for await (const message of this.serverMessages) if (message && typeof message === "object") if (LiveResponseType.SERVER_CONTENT in message) yield {
			type: "serverContent",
			...message.serverContent
		};
		else if (LiveResponseType.TOOL_CALL in message) yield {
			type: "toolCall",
			...message.toolCall
		};
		else if (LiveResponseType.TOOL_CALL_CANCELLATION in message) yield {
		...
		} else logger.warn(`Received an unknown message type from the server: ${JSON.stringify(message)}`);
		else logger.warn(`Received an invalid message from the server: ${JSON.stringify(message)}`);
	}
	/**
	* Closes this session.
	...
	*/
	async sendMediaChunks(mediaChunks) {
		...
		mediaChunks.forEach((mediaChunk) => {
			const message = { realtimeInput: { mediaChunks: [mediaChunk] } };
			this.webSocketHandler.send(JSON.stringify(message));
		});
	}
	/**
	...
	*/
	async sendMediaStream(mediaChunkStream) {
		...
		while (true) try {
			const { done, value } = await reader.read();
			if (done) break;
			else if (!value) throw new Error("Missing chunk in reader, but reader is not done.");
...
function parseDuration(duration) {
	if (!duration || !duration.endsWith("s")) return 0;
...
var LiveGenerativeModel = class extends AIModel {
	...
	*/
	async connect() {
		...
		let fullModelPath;
		if (this._apiSettings.backend.backendType === BackendType.GOOGLE_AI) fullModelPath = `projects/${this._apiSettings.project}/${this.model}`;
		else fullModelPath = `projects/${this._apiSettings.project}/locations/${this._apiSettings.location}/${this.model}`;
...
var ImagenModel = class extends AIModel {
	...
	*/
	async generateImages(prompt, singleRequestOptions) {
		...
		return handlePredictResponse(await makeRequest({
			task: "predict",
			model: this.model,
...
var WebSocketHandlerImpl = class {
	constructor() {
		if (typeof WebSocket === "undefined") throw new AIError(AIErrorCode.UNSUPPORTED, "The WebSocket API is not available in this environment. The \"Live\" feature is not supported here. It is supported in modern browser windows, Web Workers with WebSocket support, and Node >= 22.");
	}
	connect(url) {
		return new Promise((resolve, reject) => {
			...
			this.ws.addEventListener("close", (closeEvent) => {
				if (closeEvent.reason) logger.warn(`WebSocket connection closed by server. Reason: '${closeEvent.reason}'`);
			});
		});
	}
	send(data) {
		if (!this.ws || this.ws.readyState !== WebSocket.OPEN) throw new AIError(AIErrorCode.REQUEST_ERROR, "WebSocket is not open.");
		this.ws.send(data);
	}
	async *listen() {
		if (!this.ws) throw new AIError(AIErrorCode.REQUEST_ERROR, "WebSocket is not connected.");
		const messageQueue = [];
		const errorQueue = [];
		...
		const messageListener = async (event) => {
			let data;
			if (event.data instanceof Blob) data = await event.data.text();
			else if (typeof event.data === "string") data = event.data;
			else {
		...
		const closeListener = (event) => {
			if (event.reason) logger.warn(`WebSocket connection closed by the server with reason: ${event.reason}`);
			isClosed = true;
			if (resolvePromise) {
				resolvePromise();
				resolvePromise = null;
		...
		while (!isClosed) {
			if (errorQueue.length > 0) throw errorQueue.shift();
	...
	close(code, reason) {
		return new Promise((resolve) => {
			...
			if (this.ws.readyState === WebSocket.CLOSED || this.ws.readyState === WebSocket.CONNECTING) return resolve();
			if (this.ws.readyState !== WebSocket.CLOSING) this.ws.close(code, reason);
		});
	}
};
/**
...
var Schema = class {
	constructor(schemaParams) {
		if (!schemaParams.type && !schemaParams.anyOf) throw new AIError(AIErrorCode.INVALID_SCHEMA, "A schema must have either a 'type' or an 'anyOf' array of sub-schemas.");
		for (const paramKey in schemaParams) this[paramKey] = schemaParams[paramKey];
	...
	*/
	toJSON() {
		...
		for (const prop in this) if (this.hasOwnProperty(prop) && this[prop] !== void 0) {
			if (prop !== "required" || this.type === SchemaType.OBJECT) obj[prop] = this[prop];
		}
		return obj;
	}
...
var StringSchema = class extends Schema {
	...
	*/
	toJSON() {
		...
		if (this.enum) obj["enum"] = this.enum;
		return obj;
	}
};
/**
	...
	*/
	toJSON() {
		...
		if (this.optionalProperties) {
			for (const propertyKey of this.optionalProperties) if (!this.properties.hasOwnProperty(propertyKey)) throw new AIError(AIErrorCode.INVALID_SCHEMA, `Property "${propertyKey}" specified in "optionalProperties" does not exist.`);
		}
		for (const propertyKey in this.properties) if (this.properties.hasOwnProperty(propertyKey)) {
			obj.properties[propertyKey] = this.properties[propertyKey].toJSON();
			if (!this.optionalProperties.includes(propertyKey)) required.push(propertyKey);
		}
...
var AnyOfSchema = class extends Schema {
	constructor(schemaParams) {
		if (schemaParams.anyOf.length === 0) throw new AIError(AIErrorCode.INVALID_SCHEMA, "The 'anyOf' array must not be empty.");
		super({
			...schemaParams,
			type: void 0
		});
	...
	*/
	toJSON() {
		const obj = super.toJSON();
		if (this.anyOf && Array.isArray(this.anyOf)) obj.anyOf = this.anyOf.map((s) => s.toJSON());
		return obj;
	}
};
/**
...
var ImagenImageFormat = class {
	...
	*/
	static jpeg(compressionQuality) {
		if (compressionQuality && (compressionQuality < 0 || compressionQuality > 100)) logger.warn(`Invalid JPEG compression quality of ${compressionQuality} specified; the supported range is [0, 100].`);
		return {
			mimeType: "image/jpeg",
			compressionQuality
		};
...
var AudioConversationRunner = class {
	constructor(liveSession, options, deps) {
		...
		this.deps.workletNode.port.onmessage = (event) => {
			...
			const chunk = {
				mimeType: "audio/pcm",
				data: btoa(String.fromCharCode.apply(null, Array.from(new Uint8Array(pcm16.buffer))))
			};
			this.liveSession.sendAudioRealtime(chunk);
		};
	}
	/**
	...
	*/
	cleanup() {
		...
		if (this.deps.audioContext.state !== "closed") this.deps.audioContext.close();
	...
	async processPlaybackQueue() {
		...
		while (this.playbackQueue.length > 0 && !this.isStopped) {
			...
			try {
				...
				for (let i = 0; i < frameCount; i++) channelData[i] = pcm16[i] / 32768;
	...
	*/
	async runReceiveLoop() {
		...
		while (!this.isStopped) {
			const result = await Promise.race([messageGenerator.next(), this.stopDeferred.promise]);
			if (this.isStopped || !result || result.done) break;
			const message = result.value;
			if (message.type === "serverContent") {
				const serverContent = message;
				if (serverContent.interrupted) this.interruptPlayback();
			...
			} else if (message.type === "toolCall") if (!this.options.functionCallingHandler) logger.warn("Received tool call message, but StartAudioConversationOptions.functionCallingHandler is undefined. Ignoring tool call.");
			else try {
				const functionResponse = await this.options.functionCallingHandler(message.functionCalls);
				if (!this.isStopped) this.liveSession.sendFunctionResponses([functionResponse]);
			} catch (e) {
...
async function startAudioConversation(liveSession, options = {}) {
	if (liveSession.isClosed) throw new AIError(AIErrorCode.SESSION_CLOSED, "Cannot start audio conversation on a closed LiveSession.");
	if (liveSession.inConversation) throw new AIError(AIErrorCode.REQUEST_ERROR, "An audio conversation is already in progress for this session.");
	if (typeof AudioWorkletNode === "undefined" || typeof AudioContext === "undefined" || typeof navigator === "undefined" || !navigator.mediaDevices) throw new AIError(AIErrorCode.UNSUPPORTED, "Audio conversation is not supported in this environment. It requires the Web Audio API and AudioWorklet support.");
	let audioContext;
	try {
		...
		const workletNode = new AudioWorkletNode(audioContext, AUDIO_PROCESSOR_NAME, { processorOptions: { targetSampleRate: SERVER_INPUT_SAMPLE_RATE } });
		...
		if (e instanceof AIError || e instanceof DOMException) throw e;
...
function getAI(app = getApp(), options) {
	...
	const finalOptions = { useLimitedUseAppCheckTokens: options?.useLimitedUseAppCheckTokens ?? false };
	const identifier = encodeInstanceIdentifier(backend);
	const aiInstance = AIProvider.getImmediate({ identifier });
...
function getGenerativeModel(ai, modelParams, requestOptions) {
	...
	if (hybridParams.mode) {
		for (const param of Object.keys(modelParams)) if (!hybridParamKeys.includes(param)) logger.warn(`When a hybrid inference mode is specified (mode is currently set to ${hybridParams.mode}), "${param}" cannot be configured at the top level. Configuration for in-cloud and on-device must be done separately in inCloudParams and onDeviceParams. Configuration values set outside of inCloudParams and onDeviceParams will be ignored.`);
		inCloudParams = hybridParams.inCloudParams || { model: DEFAULT_HYBRID_IN_CLOUD_MODEL };
	} else inCloudParams = modelParams;
	if (!inCloudParams.model) throw new AIError(AIErrorCode.NO_MODEL, `Must provide a model name. Example: getGenerativeModel({ model: 'my-model-name' })`);
	/**
...
function getImagenModel(ai, modelParams, requestOptions) {
	if (!modelParams.model) throw new AIError(AIErrorCode.NO_MODEL, `Must provide a model name. Example: getImagenModel({ model: 'my-model-name' })`);
...
function getLiveGenerativeModel(ai, modelParams) {
	if (!modelParams.model) throw new AIError(AIErrorCode.NO_MODEL, `Must provide a model name for getLiveGenerativeModel. Example: getLiveGenerativeModel(ai, { model: 'my-model-name' })`);
	return new LiveGenerativeModel(ai, modelParams, new WebSocketHandlerImpl());
}
/**
...
*/
function registerAI() {
	_registerComponent(new Component(AI_TYPE, factory, "PUBLIC").setMultipleInstances(true));

...
const AgenticInsight: React.FC = () => {
  ...
  return (
    <section className="py-20 md:py-36 bg-white relative overflow-hidden">
      <div className="max-w-7xl mx-auto px-6 md:px-10 animate-fade-in relative z-10">
        <div className="text-center mb-16 md:mb-20">
          ...
          <p className="text-lg md:text-xl text-slate-500 max-w-2xl mx-auto leading-relaxed">
            Competitors offer summaries. LyndaPlus offers a collaborator that works alongside you to ensure research integrity and depth.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-10">
          {pillars.map((pillar, i) => (
            <div 
              key={i} 
              className="group p-8 md:p-12 rounded-[32px] md:rounded-[40px] bg-slate-50/50 border border-slate-100 hover:bg-white hover:shadow-premium transition-all duration-500 cursor-default"
            >
              <div className="w-14 h-14 md:w-16 md:h-16 rounded-2xl bg-white shadow-sm flex items-center justify-center text-2xl md:text-3xl mb-6 md:mb-8 group-hover:scale-110 group-hover:rotate-3 transition-transform duration-500">
                {pillar.icon}
              </div>
              <h3 className="text-xl md:text-2xl font-display font-bold text-slate-900 mb-4 tracking-tight">
                {pillar.title}
              </h3>
              <p className="text-[14px] md:text-[15px] text-slate-500 leading-relaxed font-medium">
                {pillar.description}
              </p>
